...
const Ourteam = () => {
  ...
  const getCount = async () => {
    try {
      let data = await fetch(`${url}/ourteam`);
      data = await data.json();
  ...
  const getTestimonials = async () => {
    try {
      let data = await fetch(`${url}/ourteam?page=${page}`);
      data = await data.json();
    ...
    <Box p="4">
      <Flex gap={5} justifyContent={"space-between"}>
        <Button
          ...
          onClick={() => navigate("/admin/ourteam/add")}
        >
          Add New
        </Button>
        {/* <Box>
                  ...
                  <Td>
                    <ButtonGroup>
                      <Button
                        ...
                        variant="solid"
                        color="#add8e6"
                        onClick={() => navigate(`/admin/ourteam/${e._id}`)}
                      >
                        View
                      </Button>
                      <Button
                        ...
                        _hover={{ bgColor: "#add8e6", color: "black" }}
                        onClick={() =>
                          navigate(`/admin/ourteam/edit/${e._id}`)
                        }
                      >
                        Edit
                      </Button>
